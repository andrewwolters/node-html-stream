...
var HtmlStream = module.exports = function() {
    ...
    Object.defineProperty(stream, 'buf', { value: '', writable: true });
    Object.defineProperty(stream, 'writable', { value: true });

    WritableStream.call(stream);

    stream.on('finish', stream._endParse.bind(stream));

    return stream;
}

util.inherits(HtmlStream, WritableStream);

HtmlStream.prototype._write = function(html, encoding, cb) {
    this._parse(html.toString());
    cb();
};
...
HtmlStream.prototype._endParse = function() {
    if (this.buf)
        throw new Error("Parse Error: " + this.buf);
