...
var HtmlStream = module.exports = function() {
    ...
    this.events = [];
    this.stack = [];
    this.stack.last = function() { return this[this.length - 1]; };
    this.buf = '';
}
    ...
    while (html) {
        ...
        if (!this.stack.last() || !elements.Special[this.stack.last()]) {
            ...
            if (html.substring(0, 4) == "<!--") {
                index = html.indexOf("-->");
                if (index >= 0) {
                    this.events.push(Event('COMMENT', html.substring(4, index)));
            ...
            if (chars) {
                ...
                this.events.push(Event('TEXT', text));
            }
        } else {
...
HtmlStream.prototype._parseStartTag = function(tag, name, rest, unary) {
    ...
    this.events.push(Event('START', Tag(name, attrs, unary)));
};
...
HtmlStream.prototype._parseEndTag = function(tag, name) {
    ...
    if (pos >= 0) {
        ...
        for (var i = this.stack.length - 1; i >= pos; i--) {
            ...
            this.events.push(Event('END', Tag(tag)));
        }
